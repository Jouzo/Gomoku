...
def capture(matrice, player, X, Y):
    ...
    for i in range(8):
        ...
        if 0 <= x1 < BOARD_LEN and 0 <= y1 < BOARD_LEN and matrice[y1][x1] == player ^ 3:
            x2, y2 = get_move(X, Y, 2, i)
            if 0 <= x2 < BOARD_LEN and 0 <= y2 < BOARD_LEN and matrice[y2][x2] == player ^ 3:
                x3, y3 = get_move(X, Y, 3, i)
                if 0 <= x3 < BOARD_LEN and 0 <= y3 < BOARD_LEN and matrice[y3][x3] == player:
                    captures += [(x1, y1, player ^ 3), (x2, y2, player ^ 3)]
    return captures